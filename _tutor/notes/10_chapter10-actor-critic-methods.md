...
### 8. 一个最小数值例子：actor 直接把动作往更好方向推

考虑一维连续动作。设 actor 很简单：
...
在当前目标动作 $a=\mu(s,\theta_t)=1.5$ 处：

$$
\nabla_aq(s,a,w)\big|_{a=1.5}
=
-2(1.5-2)
=1.
$$
...
+
\alpha_\theta
\nabla_\theta\mu(s,\theta_t)
\nabla_aq(s,a,w)\big|_{a=\mu(s)}\\
&=
1.5+0.1\times1\times1\\
&=
1.6.
...
读法：critic 说“在 $a=1.5$ 这里，动作再往右一点，价值会上升”，actor 就把输出动作从 $1.5$ 推到 $1.6$。

如果当前 $\theta_t=2.3$，则：

$$
\nabla_aq(s,a,w)\big|_{a=2.3}
=
-2(2.3-2)
=-0.6.
$$
...
$$
\theta_{t+1}=2.3+0.1\times(-0.6)=2.24.
$$

所以 deterministic actor-critic 的动作更新不是“增加某个动作概率”，而是“直接移动动作数值”。
