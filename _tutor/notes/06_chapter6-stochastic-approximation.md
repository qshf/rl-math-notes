...
**要解决的问题**：这一节不是再讲新算法，而是把第 6 章的主线收束成一句话：我们为什么要在强化学习书里插入 Robbins-Monro、Dvoretzky 定理和 SGD 这些看似“优化/概率论”的工具？

第 6 章的真正主题是：

$$
\text{在只能看到随机样本、看不到精确期望或精确函数的情况下，怎样做可靠的迭代更新？}
$$

这句话就是 stochastic approximation 随机近似 的核心。它不是一个单独算法名，而是一大类算法思想：目标对象可能是期望、均值、根、最优点，但每次只能拿到一个带噪声的观测，于是用逐步变小的步长一点点逼近目标。

### 本章的三层关系

作者在 summary 里把三个对象串了起来：
...
读法：均值估计可以看成 SGD 的一个特例；SGD 又可以看成 RM 的一个特例；最外层的统一语言是 Robbins-Monro stochastic approximation。

这条从属链很重要，因为第 7 章的 temporal-difference learning 时序差分学习 会长得像均值估计，但它背后的收敛语言来自 RM。

### 第一层：均值估计是最小的随机迭代模型

6.1 先从 mean estimation 均值估计 开始。要估计：
...
读法：旧估计 $w_k$ 不全部推翻，只向新样本 $x_k$ 的方向走一小步；$\alpha_k$ 控制这一步走多远。

这就是全书第一次正式出现的 stochastic iterative algorithm 随机迭代算法。它看起来很朴素，但后面 TD 的形状会和它非常像：
...
\text{旧估计}
+
\text{步长}\times(\text{随机目标}-\text{旧估计}).
$$

### 第二层：RM 把“估均值”推广成“带噪求根”

RM algorithm 处理的是 root-finding 求根：

$$
g(w)=0.
$$

如果能直接算 $g(w)$，我们可以用普通数值方法。但 RM 面对的是更艰难也更贴近 RL 的情况：真实函数 $g(w)$ 看不到，只能看到带噪声的输出：
...
读法：如果观测值 $\tilde g(w_k,\eta_k)$ 为正，就往负方向走；如果为负，就往正方向走；每次只相信它一点点，因为里面混着随机噪声。

RM 的优势正是原书 summary 强调的那句：它不要求你知道 objective function 目标函数 的解析表达式，也不要求你知道导数。只要你能输入 $w$，得到一个随机输出 $\tilde g(w,\eta)$，就可以做迭代。

这和强化学习的处境天然一致：环境转移概率、回报分布、真实价值函数通常都不知道；我们只有采样得到的一段轨迹、一个奖励、一个下一个状态。

### 第三层：SGD 是 RM 用在梯度方程上的特例

6.4 又把 optimization 优化 接进来。优化问题是：
...
读法：单样本梯度 = 真实平均梯度 + 随机偏差。于是 SGD 不是和 RM 无关的新东西，而是：

$$
\text{SGD is RM applied to } \nabla_wJ(w)=0.
$$

### 一个最小数值例子：从均值估计看第 6 章的全部直觉

假设随机变量 $X$ 的真实均值是 10，但我们不知道，只看到样本：

$$
x_1=6,\quad x_2=14,\quad x_3=11,\quad x_4=9.
$$

用式 (6.4)，取 $w_1=0$、$\alpha_k=1/k$：

| $k$ | 样本 $x_k$ | 更新式 | 新估计 |
|---:|---:|---|---:|
| 1 | 6 | $w_2=0+1(6-0)$ | 6 |
| 2 | 14 | $w_3=6+\frac12(14-6)$ | 10 |
| 3 | 11 | $w_4=10+\frac13(11-10)$ | 10.33 |
| 4 | 9 | $w_5=10.33+\frac14(9-10.33)$ | 10.00 |

这里每个样本都很粗糙，单看一个样本会偏高或偏低；但步长逐渐变小，估计值会稳定下来。这就是第 6 章反复研究的模式：
...
第 7 章会讲 temporal-difference learning。它的典型更新会像：
...
\alpha
\left[
\underbrace{R_{t+1}+\gamma V(S_{t+1})}_{\text{随机目标}}
-
\underbrace{V(S_t)}_{\text{当前估计}}
\right].
$$
...
\text{旧估计}
+
\alpha(\text{随机目标}-\text{旧估计}).
$$

这和 6.1 的均值估计式 (6.4) 是同一种骨架。因此第 6 章的价值不是“多讲了一个 RL 算法”，而是先给你一套判断 TD 这类算法的语言：

- 随机目标是不是无偏或近似无偏？
- 步长是否满足收敛条件？
- 噪声会不会随着迭代被压下去？
- 当前更新能不能写成某个 RM/SGD 形式？

### 四句要记住的话

1. **RM 的杀手锏**：它不需要目标函数表达式，也不需要导数；只需要能观察到输入 $w$ 对应的随机输出 $\tilde g(w,\eta)$。
2. **SGD 是 RM 的特例**：把 $\min_wJ(w)$ 转成 $\nabla_wJ(w)=0$，再把真实梯度换成单样本随机梯度，就得到 SGD。
3. **均值估计是本章的原型问题**：因为 state value 状态值 和 action value 动作值 本来就是随机变量回报的均值。
4. **式 (6.4) 是第 7 章的预告片**：
...
它是均值估计的增量形式，也是 SGD 的特例；第 7 章 TD learning 的更新会沿用这个“旧估计 + 步长 × 误差”的形状。

最后，原书还补了一句历史背景：stochastic approximation 这个名字最早由 Robbins 和 Monro 在 1951 年使用。也就是说，本章的名字其实来自 RM 的经典论文传统。

⚠️ **不要把第 6 章当成“一个新的强化学习算法章”。** 它没有策略改进、没有 grid world 实验、没有新的价值函数定义。它的作用是给后面的 TD、函数近似、策略梯度准备一套共同数学语言：在采样噪声下如何迭代、如何选择步长、以及为什么某些算法可能收敛。

---

## 6.6 Q&A（章末问答）

**要解决的问题**：章末 Q&A 用问答方式确认你有没有抓住本章主线。它不是考细节，而是在问：你能不能把“随机近似、均值估计、RM、SGD、TD 的预备关系”串成一张图。

### Q1：什么是 stochastic approximation 随机近似？

原书答案：随机近似是一大类 stochastic iterative algorithms 随机迭代算法，用来解决 root-finding 求根 或 optimization 优化 问题。

拆开说，它有三个关键词。

第一，**stochastic 随机**：每一步拿到的信息不是精确值，而是带噪声的样本。比如我们想要 $\mathbb E[X]$，但每次只看到一个 $x_k$；想要 $\nabla J(w)$，但每次只看到一个单样本梯度 $\nabla f(w,x_k)$。

第二，**approximation 近似**：算法不指望一步算出精确答案，而是通过序列
...
第三，**root-finding / optimization 求根或优化**：RM 的基本问题是
...
$$
\text{用带噪样本做迭代，去逼近根或最优点。}
$$

### Q2：为什么强化学习要学 stochastic approximation？

因为第 7 章的 TD learning 可以看成 stochastic approximation algorithm 随机近似算法。

强化学习里的困难是：价值函数常常是一个期望。例如 state value 状态值：

$$
v_\pi(s)=\mathbb E_\pi[G_t\mid S_t=s].
$$

读法：状态 $s$ 的价值，是从这个状态出发、按策略 $\pi$ 行动时，未来回报 $G_t$ 的条件期望。

问题在于，这个期望通常没法直接算。我们只能采样一条轨迹，看到一个奖励、一个下个状态，然后用样本构造更新目标。TD 的思路就是：
...
这和第 6 章的随机近似完全同频。因此先学第 6 章，看到第 7 章的 TD 更新时就不会觉得突兀：TD 不是凭空冒出来的技巧，而是“随机迭代估计期望”的强化学习版本。

### Q3：为什么本章频繁讨论 mean estimation 均值估计？

因为强化学习里的 value 价值 本质上就是 mean 均值。
...
这两个式子都在说：价值 = 某个随机回报的期望。

所以估计价值，其实就是估计均值。只不过这个均值不是普通数据集里的静态均值，而是由环境、策略、状态转移、折扣回报共同诱导出来的均值。

最小例子：

假设在某个状态 $s$ 出发，观察到三次回报：

$$
G^{(1)}=4,\quad G^{(2)}=10,\quad G^{(3)}=7.
$$
...
|---:|---:|---:|
| 1 | 4 | $V(s)=4$ |
| 2 | 10 | $V(s)=4+\frac12(10-4)=7$ |
| 3 | 7 | $V(s)=7+\frac13(7-7)=7$ |

这就是“用回报样本估计价值均值”。TD 以后会进一步大胆：它不等完整回报 $G_t$ 出来，而是用一步奖励加上下一个状态的当前价值估计来构造随机目标。

⚠️ **这问是本章和强化学习主线的扣环。** 不要只记“均值估计是例子”，要记“价值函数本来就是均值，所以均值估计是价值学习的最小模型”。

### Q4：RM algorithm 相比其他求根算法强在哪里？

RM 的优势是 black-box 黑盒。

很多求根方法需要知道函数表达式，甚至还需要导数。例如牛顿法通常要用：

$$
w_{k+1}
=
w_k-\frac{g(w_k)}{g'(w_k)}.
$$

它要求你能算 $g(w_k)$，还要能算 $g'(w_k)$。

但 RM 的设定更弱。它只要求你能观察到：
...
读法：我不知道精确函数 $g$，也不知道导数；我只拿一个带噪声的函数输出，然后用小步长修正当前估计。

这对 RL 特别重要。因为在 RL 里，我们经常不知道环境模型：

$$
p(s',r\mid s,a)
$$

也不知道真实 Bellman expectation operator 的精确期望，只能从交互中采样。RM 的黑盒性质，正好适配这种“模型未知但可以采样”的场景。

SGD 也是这个意义上的 RM 特例：它不需要完整期望梯度，只用单样本梯度。
...
但现实里，分布往往未知，期望算不出来。SGD 的做法是：用一个样本 $x_k$ 的梯度替代真实期望梯度：
...
读法：不知道总体平均下降方向，就用当前样本给出的下降方向先走一步。
...
\underbrace{\mathbb E[\nabla_w f(w_k,X)]}_{\text{真实梯度}}
+
\underbrace{\left(\nabla_wf(w_k,x_k)-\mathbb E[\nabla_w f(w_k,X)]\right)}_{\text{随机噪声}}.
$$

所以 SGD 的本质不是“乱走”，而是“真实梯度 + 零均值噪声”的迭代。只要步长和噪声条件合适，噪声可以被长期平均掉。

### Q6：SGD 能很快收敛吗？

原书说 SGD 有一个有趣的收敛模式：离最优解远时通常比较快；接近最优解后，随机梯度里的噪声变得显著，收敛速度下降。

直觉是这样的。假设真实梯度很大，噪声相对不重要：

$$
\text{真实梯度}=100,\qquad \text{噪声}=5.
$$
...
$$
\text{真实梯度}=0.5,\qquad \text{噪声}=5.
$$

这时随机噪声可能比真实信号还大，更新就会开始抖动。于是 SGD 常见图像是：前期快速下降，后期在最优点附近抖动、慢慢磨。

这也是为什么步长 $\alpha_k$ 重要。步长太大，后期抖动压不住；步长太小，前期学习又太慢。第 6 章反复出现的条件：
...
- $\sum_k a_k=\infty$：总步长不能太短，否则还没走到目标就停了。
- $\sum_k a_k^2<\infty$：平方步长总量要有限，这样噪声的累计影响能被压住。

⚠️ **SGD 的“快”不是单调快到底。** 它常常是远处快、近处慢；近处慢不是算法坏了，而是随机梯度噪声开始主导。

### Q7：什么是 MBGD？它相比 SGD 和 BGD 有什么优势？

MBGD 是 mini-batch gradient descent 小批量梯度下降。它介于 SGD 和 BGD 之间。

三者可以统一写成：

| 方法 | 每次用多少样本 | 更新方向 | 特点 |
|---|---:|---|---|
| BGD | 全部 $n$ 个样本 | 完整平均梯度 | 稳，但每步贵 |
| SGD | 1 个样本 | 单样本梯度 | 便宜，但噪声大 |
| MBGD | $m$ 个样本，$1<m<n$ | 小批量平均梯度 | 折中，实践常用 |
...
其中 $\mathcal I_k$ 是第 $k$ 次更新抽到的小批量样本索引集合。

读法：SGD 每次只听一个样本的意见；BGD 每次听完整数据集的意见；MBGD 每次听一小组样本的平均意见。

MBGD 相比 SGD，随机性更小，因为多个样本平均后会抵消一部分噪声。相比 BGD，它更灵活，因为不需要每一步扫完整数据集。

### 章末 Q&A 一张速记表

| 问题 | 一句话答案 | 和后文的关系 |
|---|---|---|
| 什么是随机近似？ | 用带噪样本迭代求根或优化 | 给 TD 提供数学语言 |
| 为什么要学？ | 第 7 章 TD 可看成随机近似 | 读 TD 时不突兀 |
| 为什么讲均值估计？ | value 本质是 return 的均值 | 价值学习就是估均值 |
| RM 强在哪？ | 不要函数表达式和导数，只要黑盒输出 | 适合模型未知环境 |
| SGD 思想？ | 用单样本梯度替代期望梯度 | RM 的优化版本 |
| SGD 快吗？ | 远处快，近处受噪声影响变慢 | 解释后期抖动 |
| MBGD 是什么？ | SGD 和 BGD 的折中 | 降低噪声又控制成本 |

本章最终要带走的不是某个公式，而是这一条链：
