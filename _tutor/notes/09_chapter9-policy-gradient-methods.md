...
#### 和 $v_\pi(s)$ 到底差在哪？一个两状态例子

式 (9.1) 和状态值函数确实很像，因为它们本质上都在取“折扣回报的期望”。区别是：
...
问的是：**这个参数策略 $\pi_\theta$ 在整个任务里的平均表现是多少？**
...
J(\theta)
=
\sum_s \rho(s)v_{\pi_\theta}(s)
=
\mathbb E_{S_0\sim\rho}\left[v_{\pi_\theta}(S_0)\right].
$$

举个最小例子。假设只有两个可能初始状态 $A,B$，某个策略 $\pi_\theta$ 下：

$$
v_{\pi_\theta}(A)=10,\qquad v_{\pi_\theta}(B)=2.
$$
...
| 起点 | 状态值含义 |
|---|---|
| $A$ | 明确从 $A$ 出发，期望总回报是 $10$ |
| $B$ | 明确从 $B$ 出发，期望总回报是 $2$ |

如果任务规定初始状态均匀出现：

$$
\rho(A)=0.5,\qquad \rho(B)=0.5,
$$
...
J(\theta)
=
0.5v_{\pi_\theta}(A)+0.5v_{\pi_\theta}(B)
=
...
J(\theta)
=
0.9\times10+0.1\times2
=
9.2.
$$

同一个策略、同样的 $v_{\pi_\theta}(A),v_{\pi_\theta}(B)$，$J(\theta)$ 会因为初始状态分布不同而改变。  
所以一句话记：

$$
\boxed{v_\pi(s)\text{ 是指定起点的价值； }J(\theta)\text{ 是按任务起点分布平均后的策略总分。}}
$$
